...
{
	$sql="UPDATE links SET verify=1 WHERE ID=".$_GET['id'];
	$conn->query($sql);
}
?>
...
<ul>
<?php
	$sql="SELECT * FROM links WHERE verify=0";
	$result=$conn->query($sql);
	while ($row=$result->fetch_array())
	{
		echo "<li><a href='".$row['linkAddress']."'></a>".$row['linkName']."</li>";
	}
?>
</ul>
<!-- InstanceEndEditable -->
